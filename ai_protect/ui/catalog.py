...
CATALOG: dict[str, dict] = {
    ...
    "manifest_validator": {
        ...
        "source_url": "https://github.com/n08976/ai-protect/blob/main/ai_protect/adapters/manifest_validator.py",
        "stages": "intake",
    ...
    "threat_model_check": {
        ...
        "source_url": "https://github.com/n08976/ai-protect/blob/main/ai_protect/adapters/threat_model_check.py",
        "stages": "design",
        "category": "Policy gates",
        "kind": "policy",
    },
    "intel_match": {
        "description": "Enrichment / detection — cross-references manifest assets against ingested CVE/KEV intel feeds and emits findings for matches (KEV → critical ratchet). Always-run pre-flight regardless of scan kind.",
        "source_url": "https://github.com/n08976/ai-protect/blob/main/ai_protect/adapters/intel_match.py",
        "stages": "build",
        "category": "Policy gates",
        "kind": "policy",
    },
    "mcp_scope": {
        ...
        "source_url": "https://github.com/n08976/ai-protect/blob/main/ai_protect/adapters/mcp_scope.py",
        "stages": "preprod",
        "category": "Policy gates",
        "kind": "policy",
    },
    ...
    "gosec": {
        "description": "gosec — Go-native SAST (G101 hardcoded creds, G201/G202 SQL formatting, G304 path traversal, G4xx weak crypto, G5xx weak hashes). Graceful degrade when no Go files in source.",
    ...
    "owasp_noir": {
        "description": "OWASP Noir — attack surface enumeration via static analysis. Static counterpart to the runtime recon chain; produces an authoritative endpoint list for downstream DAST adapters.",
        "source_url": "https://github.com/owasp-noir/noir",
        "stages": "intake",
        "category": "Static analysis · secrets · deps",
        "kind": "static",
    },
    "dependency_check": {
        "description": "OWASP Dependency-Check — multi-language CVE scanner against NIST NVD (Maven / NPM / Gradle / .NET / Ruby / PHP / Python). Broader language coverage than pip_audit. Requires NVD_API_KEY env var on v9+ (NIST throttles unauthenticated pulls). Current stable: v12.2.0.",
    ...
    "nikto": {
        "description": "Nikto — web-server misconfiguration / known-vuln / dangerous-file / header scan. Fills a gap Nuclei/ZAP/Burp cover poorly.",
    ...
    "dalfox": {
        "description": "Dalfox (v3) — XSS scanner with parameter mining + DOM (AST) analysis. Confirms reflected/DOM XSS.",
    ...
    "wpscan": {
        "description": "WPScan — WordPress core/plugin/theme CVE enumeration. No-op on non-WordPress targets. Optional WPVulnDB api_token.",
    ...
    "commix": {
        "description": "commix — OS command-injection confirmation. Mutation-required.",
    ...
    "nosqli": {
        "description": "nosqli — NoSQL (Mongo operator / error / boolean / timing) injection confirmation. Mutation-required. Replaces the Py2/interactive NoSQLMap.",
    ...
    "tplmap": {
        "description": "tplmap — server-side template injection (SSTI → RCE) confirmation across Jinja2/Twig/Freemarker/ERB/etc. Mutation-required.",
    ...
    "zap": {
        "description": "OWASP ZAP — free DAST scanner via REST API. Modes: spider (URL discovery), baseline (1-min spider + passive — Tier 3/4), active (spider + active scan), full (spider + active + extended passive), api (OpenAPI/SOAP/GraphQL spec-driven — strong fit for AI gateway / MCP server APIs).",
    ...
    "ride": {
        "description": "Adobe Ride — REST/JSON API test suite runner (configurable hook). Runs an in-house Maven Ride suite that targets the AI gateway / MCP / agent runtime API surface; converts test failures to Findings. Mutation-required.",
        "source_url": "https://github.com/adobe/ride",
        "stages": "preprod",
        "category": "Dynamic · web · network",
        "kind": "dynamic",
    },
    ...
    "caldera": {
        "description": "MITRE Caldera — autonomous adversary emulation. Chained ATT&CK abilities (an adversary profile) run end-to-end via Caldera's REST API; complements Atomic (per-technique) with campaign-shaped coverage. Mutation-required.",
    ...
    "agentic_radar": {
        "description": "SplxAI Agentic Radar — SAST for agentic AI workflows (LangChain, LlamaIndex, CrewAI, Claude Agent SDK, OpenAI Assistants). Surfaces over-broad tool grants, agent-to-agent privilege escalation, PHI flow into prompts, prompt-injection-prone input handling.",
        "source_url": "https://github.com/splx-ai/agentic-radar",
        "stages": "build",
        "category": "AI red team · eval",
        "kind": "ai",
    },
    "eval_suite": {
        ...
        "source_url": "https://github.com/n08976/ai-protect/blob/main/ai_protect/adapters/eval_suite.py",
        "stages": "preprod",
        "category": "AI red team · eval",
        "kind": "ai",
    },
    ...
    "telemetry_drift": {
        ...
        "source_url": "https://github.com/n08976/ai-protect/blob/main/ai_protect/adapters/telemetry_drift.py",
        "stages": "production",
