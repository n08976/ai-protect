# `ai_protect/` — the ai-protect AI assurance pipeline
...
python -m ai_protect.cli tier ai_protect/manifests/example_clinical_assistant.yml
...
python -m ai_protect.cli --findings ~/.ai-protect/findings.jsonl run \
    ai_protect/manifests/example_clinical_assistant.yml --stage preprod

# 4. Run every stage end-to-end (stops at first gate failure)
python -m ai_protect.cli --findings ~/.ai-protect/findings.jsonl run \
    ai_protect/manifests/example_clinical_assistant.yml --all

# 5. Generate dashboards
python -m ai_protect.cli --findings ~/.ai-protect/findings.jsonl report --kind technical
python -m ai_protect.cli --findings ~/.ai-protect/findings.jsonl report --kind executive
...
python -m ai_protect.ui.server --findings ~/.ai-protect/findings.jsonl --port 3005
...
- `/settings` — schema-driven configuration persisted to `~/.ai-protect/config.json` (chmod 600 — holds PATs / GitHub App keys). Sections (each generated from `ai_protect/core/settings.py`'s `SCHEMA` — adding a knob is one Field append, no template edits):
...
**URL safety guards** (`ai_protect/core/url_safety.py`):
...
**DAST execution policies** (`ai_protect/core/dast_config.py`): a typed `DastConfig` dataclass carries the intent — `max_rps`, `max_concurrency`, `timebox_s`, `scope_prefix`, `require_scope_prefix`, `allow_active`, `allow_adversary`. Built from `settings + manifest.target` overrides. Each adapter translates the dataclass to its native CLI flags via a small in-adapter mapping:
...
## Scanning from GitHub (`ai_protect/sources/`)
...
Implementation: `ai_protect/core/auto_resolve.py:compute_and_apply()`, called by `Orchestrator.run_stage()` after the adapter loop completes. Guards (each prevents a real false-positive class):
...
python -m ai_protect.cli --findings findings.jsonl remediate <manifest> \
...
**Auto-fix coverage** is what the remediation strategies (`ai_protect/remediate/strategies/`) support — each is deterministic and re-scan-verified; everything else routes to `proposed_awaiting_human` / `no_fix` (safe by default):
...
Add a strategy: subclass `Remediator` in `strategies/`, set `handles` + `can_fix` + `propose`, register it in `ai_protect/remediate/registry.py` (order = priority).
...
## Findings sinks (`ai_protect/integrations/`)

A **sink** is a destination for normalized findings after a scan (defect tracker, SARIF/OCSF file, webhook). The layer is pluggable: subclass `FindingsSink` (`is_configured()` + `push(findings, ctx)`), register in `ai_protect/integrations/registry.py`, and it's reachable everywhere — `cli run --sink <name>`, `cli sinks`, and any future auto-push. **DefectDojo** (`ai_protect/integrations/defectdojo/`) is the first sink.

```bash
python -m ai_protect.cli sinks                                    # which sinks are configured
python -m ai_protect.cli defectdojo --dry-run                     # preview the JSON (no creds/network)
python -m ai_protect.cli run <manifest> --stage preprod --sink defectdojo   # scan + push
```
...
Every AI app or agent declares itself with a YAML manifest (`ai_protect/manifests/example_*.yml`). The manifest is the single source of truth for tiering, adapter selection, and compliance evidence.
...
Every adapter implements the same interface (`ai_protect/adapters/base.py`): preflight → run → return list of normalized `Finding` objects. The orchestrator handles the rest.
...
Implements the v2.1 four-tier framework verbatim. See `ai_protect/core/tiering.py` for the dimension scoring and forced-Tier-1 rules.
...
`ai_protect/core/policy.py` is the operating model in code. Every (tier, stage) pair maps to an ordered list of `AdapterCall` entries with `blocking: bool` and `config: dict`. Change the table to change what the team runs.
...
Every adapter produces `Finding` objects (`ai_protect/core/findings.py`). The store is append-only JSONL — adapters write, dashboards read.
...
`ai_protect/core/compliance.py` maps every finding category to HIPAA, HITRUST, NIST AI RMF, MITRE ATLAS, and FDA SaMD identifiers. The mapping is deliberately conservative — auditors prefer over-coverage.
...
ai_protect/
├── README.md                       # this file
├── cli.py                          # python -m ai_protect.cli ...
├── requirements.txt
├── core/
...
Adding a new tool is one new file under `ai_protect/adapters/` plus a registry line and a policy-table entry.

1. Create `ai_protect/adapters/yourtool.py` with a class that inherits from `Adapter`. Implement `preflight()` (raise `AdapterUnavailable` if the tool isn't installed) and `run()` (return a list of Finding objects via `self.make_finding(...)` so compliance auto-tagging fires).
2. Register it in `ai_protect/adapters/registry.py`.
3. Add it to `ai_protect/core/policy.py` for the tier × stage combinations where it should fire.
4. Add a test in `ai_protect/tests/`.
