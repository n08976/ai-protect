...
Examples:
    ...
    python -m ai_protect.cli tier manifests/example_clinical_assistant.yml

    # Run a specific stage end-to-end
    python -m ai_protect.cli run manifests/example_clinical_assistant.yml --stage preprod

    # Run every stage in sequence (stops at first gate failure)
    python -m ai_protect.cli run manifests/example_clinical_assistant.yml --all
...
# `python -m ai_protect.cli run <manifest>` writes here instead of cluttering the
